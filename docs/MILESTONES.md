...
| NEXT-3 | negative conformance | backend＋実機証拠 | 実機で壊れる既知版をエミュレーターもFAILにし、正常版→PASS、故障版→FAIL、修正版→PASSの履歴とfalse-acceptance KPI母数を増やす | **進行中**。NEXT3-0契約・KPI schema完了。旧LCD候補1件はartifact再現不能、明示的LCD fault 2件は凍結oracle不一致で`inconclusive`。SD CMD8 bad-CRC候補はA1 emulator／実機PASS、Fault B実機R1=`0x09` FAILで初のhardware-confirmed negativeとなった。NEXT3-12初回backend runは不正CRCを受理してapp PASSとなり`false_accept`を確定。限定datasetは候補4、negative母数1、正検出0、false accept 1、artifact failure 1、inconclusive 2、positive相関7系列。初回記録は凍結済み。次は新backend revisionのCMD8 CRC7 rejection実装と、同一BINの正理由FAIL＋positive回帰 |
