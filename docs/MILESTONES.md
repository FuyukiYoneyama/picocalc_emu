...
| OPT4 | micro-opt bank | **OPT4-A screening pass / bank候補。OPT4-Bはexactness passだが速度改善未確認。OPT4-Cはexactness passだが中央値1.9094%退行で不採用。exactness絶対条件、10-run A/B＋95% CI、複数候補はbank全体で評価** |
...
中央値退行のため、いずれもpromoted targetへは採用していません。OPT4-Cの実装・exactness・A/B結果は
[`OPT4_C_DECODED_OP_8BYTE.md`](OPT4_C_DECODED_OP_8BYTE.md)に記録しています。
