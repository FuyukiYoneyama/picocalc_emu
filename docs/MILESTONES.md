...
| NEXT-3 | negative conformance | backend＋実機証拠 | 実機で壊れる既知版をエミュレーターもFAILにし、正常版→PASS、故障版→FAIL、修正版→PASSの履歴とfalse-acceptance KPI母数を増やす | **進行中**。NEXT3-0契約・KPI schema完了。NEXT3-1旧LCD候補はartifact再現不能。NEXT3-2明示的LCD CS fault v1はclean clone再現後、一般利用経路のuf2loaderで実機FAILを確認したが、solid 3色FAIL・pattern 4 mismatchで凍結oracleと不一致。`inconclusive`、母数増分0、エミュレーター未実行。NEXT3-3で旧SIO bitbang RAMRDとv1の6 MHz hardware SPI RAMRDのobserver差を主要な未制御変数として特定し、writerだけをA/Bするv2契約を実装前に固定した。次はv2 A1 baseline実装 |
