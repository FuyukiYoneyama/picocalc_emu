...
NEXT-1は完了し、NEXT-2のmulticore/audio capability範囲拡張へ進みました。NEXT-2Aの
凍結backend初回runではlaunchとFIFOは合格し、WFE/SEVとIRQ_PROC1はfail-closedで不合格に
なりました。期待値を変更せず、first-run証拠を保存して原因切り分け中です。
...
| NEXT-2 | capability範囲拡張 | backend＋新規app | multicore launch/SIO FIFO/WFE-SEV/IRQと、DMA-paced PCM sample sinkを別々のfail-closed gateで実装・相関する。設定/counter合格と実PCM出力を混同しない | **NEXT-2A初回fail-closed 2026-08-09**。実Pico SDK launch/FIFO/WFE-SEV/IRQ_PROC1の固定契約を実装前に凍結。凍結backend初回runはlaunch/FIFO合格、WFE-SEV/IRQ不合格として証拠固定。期待値を維持して原因切り分け中。NEXT-2B audioは独立契約として後続 |
