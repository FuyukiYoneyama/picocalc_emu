...
| PERF-RECOVERY | 高速地点からの性能退行復旧 | **現行計画。R0・R1完了、G0クリーン検証完了、G1（CPU・multicore・割込み正確性）・G2（LCD・PIO・PSRAM正確性）・G3（DMA・audio正確性）・G4（ヘッドレス実行基盤）candidate pass。G5（保存領域・起動経路）棚卸し完了、G5-A（RAW SD・NOR flash mutation）待ち** |
