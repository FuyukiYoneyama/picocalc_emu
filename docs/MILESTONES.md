...
| NEXT-3 | negative conformance | backend＋実機証拠 | 実機で壊れる既知版をエミュレーターもFAILにし、正常版→PASS、故障版→FAIL、修正版→PASSの履歴とfalse-acceptance KPI母数を増やす | **進行中**。NEXT3-0契約・KPI schema完了。NEXT3-1旧LCD候補はartifact再現不能。NEXT3-2明示的LCD CS fault v1は凍結oracle不一致で`inconclusive`、母数増分0。NEXT3-3でobserver差を特定しv2契約を事前固定。NEXT3-4でSIO LCD capability gapをbackend `4a90864`で解消し、A1 clean clone・emulator・variant B回帰・同一UF2のuf2loader実機が全PASS。positive相関は6系列、emulator PASS→hardware FAILは0。次はfault Bの限定実装・artifact固定。Bは実機前にemulator実行禁止 |
