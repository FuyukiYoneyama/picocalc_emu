...
確認し、NEXT-2Aを正式完了しました。次は独立契約のNEXT-2B audioです。
...
| NEXT-2 | capability範囲拡張 | backend＋新規app | multicore launch/SIO FIFO/WFE-SEV/IRQと、DMA-paced PCM sample sinkを別々のfail-closed gateで実装・相関する。設定/counter合格と実PCM出力を混同しない | **NEXT-2A完了 2026-08-09・NEXT-2B契約凍結・実装未着手**。v1の最終写真は全5項目PASSだが、one-shot markerのUSB列挙競合でログ2件が0 byteとなり、契約を緩めず未完了証拠として固定。事前契約した`picocalc-multicore-r2`は同じphaseを維持してmarker blockを周期再送し、エミュレーター決定一致、clean clone、late-attach probe、core 1 fatal fail-closedに合格。同一v2 UF2の実機ログ72 blockと最終5-PASS写真を`next2-multicore-r2-hardware-20260809-01`へ固定。NEXT-2Bは`next2-audio-v2-20260809`と独立oracleを凍結済みだが、backend/app/target/evidenceは未実装。authorityは49152 stereo frames（1.024秒）、DMA timer 0の`3/15625`、TREQ 59、DMA-origin PWM5_CC sample sink hashで、R5 counters/録音は代用不可。v1 audio契約はfirmware/backend authority混同を実装前レビューで検出し、削除せずv2がsupersede |
