...
cycleの15.0233%にpost-hoc candidateを観測しました。これはsafe windowではなく、次のOPT2-C
限定prototypeの設計入力です。詳細な順序は本書の「現在の実行順序」に従います。完了済みMilestoneの記述は、
...
## 現在の実行順序（2026-08-08 OPT2-B running horizon計測反映）
...
| OPT2 | exact event batching | backend＋firmware regression | CPU running区間を含め、全boundary eventのcycle/orderを変えず、有意な性能改善を得る | **継続中**。dispatcher-only候補はrevert。OPT2-B profilerは完了し、次は限定OPT2-C prototype |
