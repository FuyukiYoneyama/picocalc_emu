...
| R4 | 品質ゲートとCI | 3リポジトリ | clean cloneから同じpinで再現する。`picotetris`はunit＋RP2040 build、backendはtest＋fmt＋Clippy、`picocalc_emu`はportable＋host＋target/schema＋firmware regressionを実行し、失敗した層を特定できる | **進行中（2026-08-06着手）**。backend品質ゲート・generator親Git継承修正完了、残りはversioned validationと2リポジトリのCI接続 |
...
1. 既存の時点証拠を書き換えず、R4 target revisionとversioned validation recordを追加する。
2. `picocalc_emu`へportable、host、target/schema、firmware regressionの層別CIを接続する。
3. `picotetris`へunit testと固定SDK/toolchainによるRP2040再現buildのCIを接続する。
4. clean cloneから各pinと成果物を再現し、3リポジトリのfull gateを通す。
