...
| NEXT-3 | negative conformance | backend＋実機証拠 | 実機で壊れる既知版をエミュレーターもFAILにし、正常版→PASS、故障版→FAIL、修正版→PASSの履歴とfalse-acceptance KPI母数を増やす | **進行中**。NEXT3-0契約・KPI schema完了。NEXT3-1旧LCD候補はartifact再現不能。NEXT3-2明示的LCD CS fault v1は凍結oracle不一致で`inconclusive`。NEXT3-3〜5でobserver固定v2を事前設計し、A1はclean clone・emulator・実機全PASS、Fault Bも再現可能artifactとして固定した。NEXT3-6実機先行runはv1と同じrotated-bit症状となり、凍結oracle不一致で`inconclusive`。NEXT3-7 source gap分析ではobserver差が十分条件でないこと、最大残差が旧160x160 tilingであること、旧build環境が回収不能であることを固定し、v2を閉じた。NEXT3-8でSD CMD8 bad-CRC候補を事前設計し、NEXT3-9のA1正常対照は2 clean build、凍結backend emulator、同一UF2のuf2loader実機runが全PASSした。候補監査3件、artifact failure 1件、inconclusive 2件、negative母数0、positive相関7系列、emulator PASS→hardware FAILは0。LCD BとSD Faultのemulator runは禁止のまま。次はidentity・marker・CMD8 CRC `0x87 -> 0x85`だけを変えたSD Fault B artifactの固定 |
