...
| 10 | **SD-GEN-1 汎用SD protocol generalization** — uf2loader以外のアプリを含む汎用SD経路 | **P1完了（source inventory／clean trace／wire契約・受入マトリクス）。次はP2最小state machine。production実装未着手。詳細は[`SD_GEN1_IMPLEMENTATION_PLAN_20260823.md`](SD_GEN1_IMPLEMENTATION_PLAN_20260823.md)と[`SD_GEN1_P1_WIRE_CONTRACT_20260823.md`](SD_GEN1_P1_WIRE_CONTRACT_20260823.md)** |
...
P0のtrace棚卸しとP1のwire契約・受入マトリクスまで完了しました。次段階の`SD-GEN-1`では、uf2loaderの固定traceに限定しないSDモデルの一般化
