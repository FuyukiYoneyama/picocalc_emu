...
| OPT4 | micro-opt bank | **OPT4-A screening pass / bank候補。OPT4-Bはexactness passだが速度改善未確認。OPT4-Cはexactness passだが中央値1.9094%退行、OPT4-Dはexactness passだが正の改善未確認で不採用。exactness絶対条件、10-run A/B＋95% CI、複数候補はbank全体で評価** |
...
中央値退行、OPT4-Dは正の改善未確認のため、いずれもpromoted targetへは採用していません。OPT4-C/Dの
