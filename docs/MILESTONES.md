...
| NEXT-3 | negative conformance | backend＋実機証拠 | 実機で壊れる既知版をエミュレーターもFAILにし、正常版→PASS、故障版→FAIL、修正版→PASSの履歴とfalse-acceptance KPI母数を増やす | **進行中**。NEXT3-0契約・KPI schema完了。NEXT3-1旧LCD候補はartifact再現不能。NEXT3-2明示的LCD CS fault v1は凍結oracle不一致で`inconclusive`。NEXT3-3〜6のobserver固定v2も実機oracle不一致で閉じた。NEXT3-8〜10でSD CMD8 bad-CRC候補を設計し、A1はemulator・実機PASS、Fault Bは最小差分・2 clean build・bundleを固定した。NEXT3-11実機先行runはCMD8 CRC=`0x85`へR1=`0x09`を返して凍結oracleへ完全一致し、初のhardware-confirmed negative caseとなった。候補監査4件、artifact failure 1件、inconclusive 2件、negative母数1（初回emulator分類pending）、positive相関7系列。LCD B emulatorは禁止。SD Faultは凍結backend初回runのみ解禁。次はその結果を変更せず保存・分類 |
