...
次の機能計画は、任意I2C外部moduleを扱うI2C-EXTです。E0・E1は完了し、E2 modelを実装中です。
...
| I2C-EXT | 任意I2C外部module（RTC/EEPROM/AHT20/BMP280） | **E0・E1完了、E2 model実装中。private hardwareを既定構成へ入れず、run単位profileとしてattach/detachする。固定証拠は[`i2c-ext-e0-20260823-01`](../firmware-validation/evidence/i2c-ext-e0-20260823-01/)、詳細は[`I2C_EXTERNAL_MODULE_EMULATION_PLAN_20260823.md`](I2C_EXTERNAL_MODULE_EMULATION_PLAN_20260823.md)。E1 backend commitは`60ac700`、E2 model coreは`f1ae8dc`** |
...
実装はE0（source/provenanceとwire contract）、E1（controller/mux/shared virtual-time）まで完了し、E2のDS3231/AT24C32 model coreを追加した。現在はE2のprofile/fixture接続で、capabilityはまだ変更しない。
