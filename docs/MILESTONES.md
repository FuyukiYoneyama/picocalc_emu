...
| NEXT-3 | negative conformance | backend＋実機証拠 | 実機で壊れる既知版をエミュレーターもFAILにし、正常版→PASS、故障版→FAIL、修正版→PASSの履歴とfalse-acceptance KPI母数を増やす | **進行中**。NEXT3-0契約・KPI schema完了。NEXT3-1旧LCD候補はartifact再現不能。NEXT3-2明示的LCD CS fault v1は凍結oracle不一致で`inconclusive`、母数増分0。NEXT3-3でobserver差を特定しv2契約を事前固定。NEXT3-4でSIO LCD capability gapをbackend `4a90864`で解消し、A1 clean clone・emulator・variant B回帰・同一UF2のuf2loader実機が全PASS。NEXT3-5でA1から3 pathだけを変更したFault B commit `3a073fb`と再現可能BIN/UF2/source bundleを固定した。positive相関は6系列、emulator PASS→hardware FAILは0。次はFault Bのuf2loader実機先行run。凍結oracle完全一致までBのemulator実行は禁止 |
