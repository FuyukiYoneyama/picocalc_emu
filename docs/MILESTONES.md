...
| NEXT-3 | negative conformance | backend＋実機証拠 | 実機で壊れる既知版をエミュレーターもFAILにし、正常版→PASS、故障版→FAIL、修正版→PASSの履歴とfalse-acceptance KPI母数を増やす | **進行中**。NEXT3-0契約・分母付きKPI schema完了。NEXT3-1旧LCD 0.3.1監査はartifact再現不能・実機未確認のため母数不採用。次は現行正常版から明示的fault版を作る |
