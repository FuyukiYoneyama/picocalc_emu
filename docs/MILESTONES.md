...
NEXT-1は完了し、次はNEXT-2のmulticore/audio capability範囲拡張です。
...
| NEXT-2 | capability範囲拡張 | backend＋新規app | multicore launch/SIO FIFO/WFE-SEV/IRQと、DMA-paced PCM sample sinkを別々のfail-closed gateで実装・相関する。設定/counter合格と実PCM出力を混同しない | **NEXT-2A開始 2026-08-09**。実Pico SDK launch/FIFO/WFE-SEV/IRQ_PROC1の固定契約を実装前に凍結。NEXT-2B audioは独立契約として後続 |
