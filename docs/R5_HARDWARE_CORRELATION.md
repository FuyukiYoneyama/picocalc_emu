...
2. PicoCalcを起動する。約1 kHzの参照音が聞こえたかを`yes`/`no`で記録する。音の確認に
   音量・音程の主観評価は加えない。
3. 67キー画面が出たら、灰色または黄色のキーだけを任意順で入力する。通常キーは押して
   離す。`UP`と`DOWN`だけはrepeatが届くまで長押ししてから離す。
4. `CAPS`は他の66キーが緑になった後、最後に押して離す。
...
- **違う順・重複入力:** 失敗にしない。確認済みキーは緑のままで、未確認キーだけ続ける。
- **releaseを取り損ねた:** 黄色の該当キーをもう一度押して離す。
- **`UP`/`DOWN`が黄色のまま:** その矢印だけをrepeatまで長押しし、離す。
- **`CAPS`を早く押した:** `PRESS CAPS AGAIN`表示中は他キーを入力せず、もう一度`CAPS`を
  押して通常状態へ戻す。その後は以前の成功を保ったまま続行し、最後に`CAPS`を検査する。
