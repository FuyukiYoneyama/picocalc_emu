...
`picocalc_helloworld`は目印であって、このリポジトリの資産ではない。各自が手元で
ClockworkPi公式リポジトリからビルドして実行する。ソースもELF/BIN/UF2も同梱しない。

文書には「動くことを確認した」という結果と、再現に必要な識別情報（source commit、
ビルド設定、SDK/toolchain、成果物のSHA-256）を記録する。確認したい人は同じ手順で
再現できる。

CIなどで配布可能なfixtureが必要になったら、公式サンプルを同梱せず、上表の契約を
満たす等価なサンプルを自作する。
...
- address `0x1f`のregister/FIFO、key state、modifier、hold/repeat、overflow policy、
  battery、backlightを、対象workloadが使う範囲で実装する。
