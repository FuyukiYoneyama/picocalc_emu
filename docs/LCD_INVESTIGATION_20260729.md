...
| 実機判定 | 不合格（`pico20260730_215753.log`、下記） |

## `pico20260730_215753.log` の判定（2026-07-30）

画面はuf2loaderのメニューのまま、電源OFF/ONではノイズ画面。ただしこのログは、この
一連の調査で初めてパネルの応答を示している。

* `RAMRD`の値が初めて非ゼロで安定した（`dummy=0x01`→`0x07`、`0x0707`／`0x0400`）
* 書き込んだ色を黒→白→赤→緑→青と変えても読み値が変わらない
* 読み出す座標を(8,8)から(32,72)へ変えると読み値が変わる（`0x0302`／`0x0400`）

読み出し経路（ビットバンSIO）で送る`CASET`/`RASET`/`RAMRD`はコントローラに届いており、
返ってくるのは残存したuf2loaderの画像だと解釈できる。パネルはリセットでGRAMを消さない
ため、「loaderから起動するとメニューが残る」「コールドブートではノイズ」という症状とも
一貫する。

したがって切り分けは次に狭まった。

* SIO（ビットバン）の書き込みはパネルに届いている
* PIO経由の書き込みは1バイトも効いていない
