...
これは[`DOGFOODING_20260805.md`](DOGFOODING_20260805.md)の穴3である。ライン消去や
衝突判定は純粋な関数なのに、試すにはRP2040イメージを作ってエミュレーターで走らせる
しかなかった。
...
エミュレーターのカードモデルと同じFAT16レイアウトを直接書き込んでいる。
...
**キーボードの31イベント上限はホスト都合ではない。** コントローラは深さを5ビットで
しか報告できず、BSPは`key_info[0] & 0x1f`として読む。32件保持したコントローラは
...
26項目を検査して1行ずつ結果を出す。テストフレームワークに依存しないので
CIからそのまま使える。
