...
E5の同一BIN emulator回帰と同一UF2の実機probeが完了した。E6では
`picocalc-clock-i2c-env-e5`をactive targetとしてversioned validationへ固定し、
`capability.json`へoptional profileのbounded capabilityを追加した。
...
5. **同一UF2を実機へ一度だけ送る（完了）**。PicoCalcの通常経路（uf2loader）で起動し、UARTへ
...
6. source/BIN/UF2/backend/profile/fixture/sidecarと実機UARTをversioned targetへ固定する
   （**E6完了**）。targetは`active`へ昇格し、`capability.json`へ
   `i2c-external-rtc-env-v1`のbounded capabilityを追加した。
