> 2026-09-11文書再編前の時点記録。現在の状態・計画ではありません。
...
一致しています。続くG3（DMA・audio正確性）は、現行`picocalc-audio-r1`が必要とするDMA timer／audio sinkを
...
詳細なコマンド、差分、判定は
`picoem-picocalc/docs/BACKEND_CHANGE_VALIDATION_PLAN.md`（当時のbackend文書。復旧後mainには存在しない）
