...
- Status (updated 2026-09-01 15:00 JST): P0-A1、P0-0、P0-B は完了。P0-A2 null-control、host-stability v1/v2/v3、CPU-time attribution は、既存の単一ゲスト・CPU 11固定・wall中心の測定経路を評価した記録として完了しているが、production A/B の採否証拠にはまだ使わない。`-jN` 型の並列負荷と混同していたため、現行 wall測定の解釈を修正し、CPU時間を主指標とする単一ゲスト測定、独立ゲスト並列のホストスケーリング診断、短い固定ブロック測定を追加する。P1-A/P2-A の実装・correctness・diagnostic profile は完了しているが、CPU identityを揃えた再取得と新測定契約での性能 A/B は未開始である。現 WSL の速度が不安定と断定して同じ長時間ループを繰り返さず、先に測定経路修正と短時間診断を閉じる。測定経路の第一段は backend commit `d8f5bb2` で実装済みで、`picocalc-run --host-timing <path>` が `run_loop` 内の process CPU time と monotonic wall time を sidecar に出力する。したがって次に行うのは38時間の再収集ではなく、短時間 pilot の実測である。
- Date: 2026-09-01
...
production A/B はまだ採否していない。v3 `interleaved-anchor-v3` null-control が有効になった後、
...
現在の P0-A2 実測: `rp2040-cpu-p0-null-v3-20260901-02` は CPU 11、同一 executable の null-control、40 measured run、9境界×3 anchor（27 anchor）、60秒 cooldown の固定条件で完走し、correctness preflight、checksum、target-schema は pass したが calibration/null-control gate は invalid となった。local residual 最大34.0891%、anchor group dispersion、pair sensitivity 最大12.3076%、PicoTetris host-corrected null effect +2.5523% を記録した。P0-A2 は未完了であり、次の実測マイルストーンは `P0-A2-HS` preflight → 新 batch v4 である。

2026-09-01 時点では P0-A1、P0-0、P0-B は pass、P0-A2 は六つの旧 immutable invalid A/B record と v3 `rp2040-cpu-p0-null-v3-20260901-02` を合わせた七つの invalid batch recordを保持している（v3 `...-01` は correctness preflightのみの partial record）。CPU 11 の `rp2040-cpu-p0-null-v2-20260831-02` は 40 run、15 anchor、correctness、checksum、target-schema、group MAD、raw/host-corrected null-effect と CI を pass したが、global log-linear anchor model の最大 residual 5.1743%（RMS 3.1993%）が固定2% stability gateを超えたため invalid である。v3も host throughput regime shift により local residual、group dispersion、pair sensitivity、PicoTetris corrected null-effect が不成立となった。非線形な host trajectoryを有効結果へ読み替えず、閾値緩和や結果の再解釈は行わない。v2の宣言したpiecewise correctionと実装gateの契約差を修正する `interleaved-anchor-v3`（9境界×3 replicate、27 anchor、piecewise log knot、leave-one-group-out local residual ≤2%、pair sensitivity ≤2%）の runner/schema/verifier/unit test と CPU11 correctness record は完了した。次は `P0-A2-HS` host-stability preflightを通してから、新 batch IDでv3条件を再実行する。
P0-A2-HS v1 sentinel `rp2040-cpu-host-stability-20260901-02` は10測定を完走したが raw隣接差分3.9720%でinvalidとなった。固定2%を維持した v2 grouped sentinel（12測定、3回×4群、群内MAD・群中央値差分 gate）も `rp2040-cpu-host-stability-20260901-03` として完走したが、group 3→4 の群中央値差分3.4183%でinvalidとなった。CPU pressure/steal/scheduler観測を加えた v3 `rp2040-cpu-host-stability-20260901-04` も完走したが、全体MAD2.0868%、group 1→2差分2.2910%でinvalidで、公開host countersは安定していた。CPU-time attribution `rp2040-cpu-time-attribution-20260901-01` も完走し、CPU-time relative MAD 4.5002%、wall relative MAD 5.4821%を記録した。対象unit test 49/49、全体222/222、target-schema 37/37 は通過済みである。現WSL hostでは production A/B を開始せず、安定hostまたは実機相当の承認済み環境を前提とする。
...
なお、P1-A/P2-A の「実装・correctness・diagnostic profile 完了」は三つの別の完了条件を指す。実装は feature-on runtime と feature forwarding を固定 commit に保存したこと、correctness は二 workload の guest projection/behavior trace/checksum と candidate identity を検証したこと、diagnostic profile は候補固有の counter と保存式を検証したことを意味する。P1-A profile の decode miss 低下や P2-A profile の no-candidate reject 99.989%/99.943% は作用点の存在を示す診断証拠であり、wall-time の改善率や production promotion の証拠ではない。production A/B に接続するには、安定 host 上で CPU 11 に揃えた correctness/profile record と有効 null-control が別途必要である。
