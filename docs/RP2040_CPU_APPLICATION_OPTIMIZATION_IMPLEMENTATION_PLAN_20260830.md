...
P1-A は runtime 実装、profile、correctness を完了し、診断 profile signal を保存済みであるが、既存 correctness/profile record は `measurement_cpu=null` のため、production A/B の CPU と一致する record ではない。P1-A の production 性能 A/B の測定・採否は有効な null-control と CPU-aligned correctness 完了後まで保留している。P2-A は `ba93c1f` で fast-reject実装とfeature testを完了し、`rp2040-cpu-p2-a-correctness-20260831-01` の両 workload correctness、behavior trace、checksumを完了した。`rp2040-cpu-p2-a-profile-20260831-01` で `cpu-application-profiler,pending-exception-fast-reject` の feature provenance、aggregate/core counter 式、no-candidate reject率 99.989%/99.943% を検証済みであるが、correctness は CPU 1、profile は CPU 0 である。性能A/Bはnull-control完了後に行う。batch 05 が pass した場合は CPU 11 を共通値として P1-A correctness、P2-A correctness、P2-A profile を取り直し、その pass record だけを pointer gate で production A/B に接続する。
