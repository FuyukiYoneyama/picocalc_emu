...
| 3 | P0-A2 null batch | v2 replicated calibration（5境界×3、median/MAD）、40-run null record、environment verification |
...
2026-08-31 時点では P0-A1、P0-0、P0-B は pass、P0-A2 は四つの invalid batchを保持している。CPU 11 の `rp2040-cpu-p0-null-20260831-05` は 40 run、correctness、checksum、target-schema、null-effect の raw/host-corrected 効果と CI を pass したが、`anchor-post-003` の局所外れにより最大 residual 4.5046% が固定 2% stability gateを超えたため invalid である。pre/post driftは1.5724%であり、全体差だけでは検出できない局所外れだった。P0-A2 は同じ閾値を維持した v2（5境界×3 replicate、中央値/MAD）を実装済みで、新 batch の correctness・null-control測定へ進む。
P1-A は runtime 実装、profile、correctness を完了し、診断 profile signal を保存済みである。既存 correctness record は `measurement_cpu=null` のため、production A/B の CPU と一致する correctness を取り直す必要がある。既存 profile は診断専用で A/B admission には使わない。P1-A の production 性能 A/B の測定・採否は有効な null-control と CPU-aligned correctness 完了後まで保留している。P2-A は `ba93c1f` で fast-reject実装とfeature testを完了し、`rp2040-cpu-p2-a-correctness-20260831-01` の両 workload correctness、behavior trace、checksumを完了した。`rp2040-cpu-p2-a-profile-20260831-01` で `cpu-application-profiler,pending-exception-fast-reject` の feature provenance、aggregate/core counter 式、no-candidate reject率 99.989%/99.943% を検証済みであるが、correctness は CPU 1、profile は CPU 0 である。性能A/Bはnull-control完了後に行う。batch 05 は anchor stability gateでinvalidだったため、v2 null-controlがpassした場合にCPU 11を共通値として P1-A correctness、P2-A correctness、P2-A profile を取り直し、その pass record だけを pointer gate で production A/B に接続する。
...
| P0-A2 | admitted baseline | v2 null batch/calibration が完全記録され、境界別 relative MAD と集約 anchor residual の双方が2%以下の host stability gate を通る | 閾値を緩和せず、replicate外れ値/host条件を切り分けて新 batch ID で全体再実行 |
