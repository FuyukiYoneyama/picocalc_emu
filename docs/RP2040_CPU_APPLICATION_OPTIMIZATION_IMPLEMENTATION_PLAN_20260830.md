...
> **現行ステータス（2026-09-02 23:34 JST）**: P0-A1、P0-0、P0-B、P0-A2-M、CPU-time主 null-control、P1-A correctness/profile、P1-B実装・CPU11 correctness/profile gateは完了。P1-A v3 CPU-time A/Bは40 run＋27 anchorを完走したが、local calibration residual 3.096448%で`invalid`として閉じた。P1-B CPU-time production A/Bは batch `rp2040-cpu-p1-b-production-v3-20260902-05`、CPU11、`interleaved-anchor-v3`、10 pair/workload、60秒 cooldownで実行中（session 11410）。P1-Bの全runとanchorが確定するまで効果判定は未確定。既存P2-A correctness/profile `...-20260901-02` は manifest の `measurement_cpu=11`、commit/build/runner identity、checksum、target-schemaを確認済みで、decisionにCPU欄がないだけなので、同一artifact provenanceを再照合してpointer再利用する。P1-A invalidの数値は候補効果へ採用しない。
