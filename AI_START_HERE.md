...
- 既存R/NEXT/U/SD-GEN番号付き作業: すべて完了または正式終了。Validated Realtime PreviewはVRP-0〜VRP-3まで完了し、実targetの完全digest gate、Tk GUI、PicoCalc skin、LCD、UART0 console、入力、reset/reloadをローカル／WSLgで受入済み。VRP-4以降は未着手
...
- Validated Realtime Preview: **VRP-0／VRP-1／VRP-2-a〜e／VRP-3完了（VRP-2-e実target gate、VRP-3 GUIを受入済み）**。`python3 tools/picocalc.py test --mode firmware --receipt-out ...`でPASS済みrunのreceiptを生成し、`python3 tools/picocalc.py preview ...`でadmissionを再検証できます。`preview-digest-gate`はclean backend `c1c20d7d86a3006569375bc333cf72494e95eb46`で、`picotetris-opt1b-vrp2f` revision 8と`picoedit-r1-vrp2f` revision 4のbatch／machine API／preview API／registered report四者projection digestを実BIN・fresh audio reportとともに受入しました。`python3 tools/picocalc.py preview-gui --descriptor ...`は同じdescriptorからTk薄型frontendを起動し、PicoCalc skinへLCDを合成してUART0 consoleを自動表示します。key down/held/up・auto-repeat抑止、F5 reset、Ctrl+R再admission reload、F12 screenshot、sticky UX-invalidを含む詳細は[`docs/validated-realtime-preview/VRP3_GUI_20260829.md`](docs/validated-realtime-preview/VRP3_GUI_20260829.md)にあります。各証拠は[`firmware-validation/records/`](firmware-validation/records/)と[`docs/validated-realtime-preview/`](docs/validated-realtime-preview/)にあります。これはhost audio transport、実機相関、または1倍qualifiedを意味しません。次はVRP-4 audio、VRP-5 qualification、VRP-6 capability/docsです。正式な1倍qualified昇格にはVRP-NES-0のNES-class targetが追加で必要です
