...
- 既存R/NEXT/U/SD-GEN番号付き作業: すべて完了または正式終了。Validated Realtime PreviewはVRP-2-a〜eまで完了し、実targetの完全digest gateも受入済み。VRP-3以降は未着手
...
- Validated Realtime Preview: **VRP-0／VRP-1／VRP-2-a〜e完了（VRP-2-e実target gate受入済み）**。`python3 tools/picocalc.py test --mode firmware --receipt-out ...`でPASS済みrunのreceiptを生成し、`python3 tools/picocalc.py preview ...`でadmissionを再検証できます。`preview-digest-gate`はclean backend `c1c20d7d86a3006569375bc333cf72494e95eb46`で、`picotetris-opt1b-vrp2f` revision 8と`picoedit-r1-vrp2f` revision 4のbatch／machine API／preview API／registered report四者projection digestを実BIN・fresh audio reportとともに受入しました。各証拠は[`firmware-validation/records/`](firmware-validation/records/)と[`docs/validated-realtime-preview/`](docs/validated-realtime-preview/)にあります。これはGUI、host audio transport、実機相関、または1倍qualifiedを意味しません。次はVRP-3 GUI/input、VRP-4 audio、VRP-5 qualification、VRP-6 capability/docsです。正式な1倍qualified昇格にはVRP-NES-0のNES-class targetが追加で必要です
...
実targetへ接続した完全digest gateも完了しています。GUI、audio、1倍qualificationは未完了です。受入済みtargetと証拠は[`docs/validated-realtime-preview/VRP2E_REGISTERED_DIGEST_GATE_20260829.md`](docs/validated-realtime-preview/VRP2E_REGISTERED_DIGEST_GATE_20260829.md)を参照してください。
