...
- 既存R/NEXT/U/SD-GEN番号付き作業: すべて完了または正式終了。Validated Realtime PreviewはVRP-0〜VRP-4まで完了し、実targetの完全digest gate、Tk GUI、PicoCalc skin、LCD、UART0 console、入力、reset/reload、bounded host audio monitorのformal evidenceをローカル／WSLgで受入済み。VRP-5以降は未着手で、1倍qualification用のrepository-owned `VRP-LOAD-0`は未実装・未測定
...
- Validated Realtime Preview: **VRP-0／VRP-1／VRP-2-a〜e／VRP-3／VRP-4完了（VRP-2-e実target gate、VRP-3 GUI、VRP-4 host audio monitor formal gateを受入済み）**。`python3 tools/picocalc.py test --mode firmware --receipt-out ...`でPASS済みrunのreceiptを生成し、`python3 tools/picocalc.py preview ...`でadmissionを再検証できます。`preview-digest-gate`はclean backend `c1c20d7d86a3006569375bc333cf72494e95eb46`で、`picotetris-opt1b-vrp2f` revision 8と`picoedit-r1-vrp2f` revision 4のbatch／machine API／preview API／registered report四者projection digestを実BIN・fresh audio reportとともに受入しました。`python3 tools/picocalc.py preview-gui --descriptor ...`は同じdescriptorからTk薄型frontendを起動し、PicoCalc skinへLCDを合成してUART0 consoleを自動表示します。key down/held/up・auto-repeat抑止、F5 reset、Ctrl+R再admission reload、F12 screenshot、sticky UX-invalidを含む詳細は[`docs/validated-realtime-preview/VRP3_GUI_20260829.md`](docs/validated-realtime-preview/VRP3_GUI_20260829.md)に、bounded PCM monitorとformal off／on／forced-drop evidenceは[`docs/validated-realtime-preview/VRP4_AUDIO_MONITOR_20260829.md`](docs/validated-realtime-preview/VRP4_AUDIO_MONITOR_20260829.md)と[`firmware-validation/records/vrp4-picotetris-20260829-01/`](firmware-validation/records/vrp4-picotetris-20260829-01/)にあります。これはhost audio transportの検証であり、実機相関、hardware-audio fidelity、または1倍qualifiedを意味しません。`VRP-LOAD-0`のrepository-owned sustained-load計画をVRP-5の正式qualification前提として追加しました。既存VRP-NES-0のfixture／target／evidenceは歴史資料として保持し、NEScoをVRP-5の依存にはしません

VRP-2-eの`c1c20d7d86a3006569375bc333cf72494e95eb46`は当時の不変evidenceとして保持しますが、現在のbackend branch／tagから到達できません。VRP-5ではこのpinを再利用せず、到達可能なclean backendで新しいversioned target／validation／receiptを作る必要があります。`VRP-LOAD-0`のsource／fixture prototypeは並行できますが、formal qualificationはこのpin preflight後です。
...
`--i2c-profile picocalc-rtc-v1 --i2c-report <path>`、または環境sensorを含める場合は
`--i2c-profile picocalc-rtc-env-v1 --i2c-report <path>`を渡します。fixtureを使う場合は
`--i2c-fixture <fixture.json>`も指定します。profile runのsidecarは
[`firmware-validation/i2c-ext-report.schema.json`](firmware-validation/i2c-ext-report.schema.json)
のschema 2で、transaction digest、各deviceのread/write/STOP、protocol error、最終stateを含みます。
versioned targetから実行するときは、`picocalc.py test --mode firmware`がtargetに固定されたprofile・
fixture SHA・sidecar checksを自動的にrunnerへ渡します。profileを省略した通常runには
