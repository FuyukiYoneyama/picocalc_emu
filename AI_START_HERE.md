...
| 2 | エミュレーター — PC上で実行し、画面・SPI/I²C・SDをあなた自身が観測する | **公式サンプル（A系統）は縦断完了**（HELLO-FULL、2026-08-04）。標準templateのB系統は**未対応** | **あなたのアプリはまだ自分で確認できない。人間にログと写真を依頼する** |
| 3 | 実機相関 — 実機結果を台帳へ記録し、予測精度を校正する | 一部 | 実機結果は必ず台帳へ記録する |

第2段は、ClockworkPi公式サンプル`picocalc_helloworld`（LCD A系統: SPI1/RGB666）を
エミュレーター上で完全に実行できるところまで到達しました（HELLO-FULL、2026-08-04）。
LCD描画、8 MiB PSRAM全域試験、I2Cキーボード、キー入力のechoまでPC上で観測できます。

**ただし、これはあなたが書くアプリの検証にはまだ使えません。** 標準templateが使う
B系統（PIO0/RGB565/LCD DMA OFF）はエミュレーター側が未対応で、その対応は
Gate 7の作業です。したがって**あなたは現時点でも、自分の変更が正しいかを自力で
確認できません。** 推測でハードウェア層を触らないことと、人間へ渡す1回のUF2の
情報量を最大にすること（版の識別、機械可読ログ、合否の判定基準を先に決めておくこと）が、
現在のあなたの最重要責務です。
