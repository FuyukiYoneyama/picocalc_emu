...
> **現在の位置付け:** BSP・生成・host検証に加え、別repoのRP2040 firmware backendでraw BINを実行します。
> 使用版と機能範囲は[現在の実装状況](docs/IMPLEMENTATION_STATUS.md)、全体構成は
> [プロジェクト体系](docs/PROJECT_OVERVIEW.md)を参照してください。高速化・UX 1倍速は失敗・終了した履歴です。
...
削減対象は人間の作業である。PC上でアプリの挙動を再現し、推測による修正と実機確認の往復を減らす。
承認されたHIL runnerで補完できる操作と、人間による表示・聴感・物理操作の確認を区別する。
以降の要求は、この開発の反復を支えるための手段として読むこと。
