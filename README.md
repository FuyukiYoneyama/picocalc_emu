...
- Validated Realtime Preview: **VRP-0／VRP-1／VRP-2-a〜d完了、VRP-2-e完全digest gate実装済み、実target受入が残る**。`picocalc.py preview`は権威あるfirmware PASS済みの同一BIN・同一runnerを再検証し、admitted descriptorを出力します。backendの`--preview-api`は固定PCRP IPC、UART0 TX/RX、RGB565 frame、reset/quit、pacer status、fail-closed入力を実装済みで、batch／machine API／preview APIのsessionはbackendの`src/session.rs`へ共有分離済みです。preview statusとmachine APIの`preview`観測domainにはUART／framebuffer／unsupported-MMIO／audio-sinkのversioned projection/digestがあり、board-backed synthetic UART fixtureについてbatch／machine／previewを同一cycleで比較するreport-compatible observation digest smoke gate（初期RGB565 LCD frameを含む）をローカル確認済みです。VRP-2-aでは現行backendのversioned target／validation recordを追加し、2 workloadのreceipt／registered-target admissionをローカル確認済みです。VRP-2-bではadmitted descriptorのlaunch contractを再検証して同じrunnerのhello／status／quitをheadless consumerで確認済みです。VRP-2-cではmachine API schema-1の8交換golden transcript、VRP-2-dではUART RX accepted／echo／FIFO overrunの方向付きfixtureをローカル確認済みです。VRP-2-eの`preview-digest-gate`はdescriptorからbatch／machine／preview／registered reportの完全projection digestを比較する実装とfake-target検査まで完了していますが、現行registered reportはcomplete audio observation作成前で、実BINを含む実target受入は未完了です。実targetの完全gateを完了するまでpreview機能をsupportedとみなしません。VRP-2〜VRP-4は`picotetris-opt1b`＋`picoedit-r1`で進め、正式な1倍qualified昇格には別途NES-class target（VRP-NES-0）が必要です。契約fixtureとVRP-2記録は[`docs/validated-realtime-preview/`](docs/validated-realtime-preview/)に、現行計画は[`docs/VALIDATED_REALTIME_PREVIEW_IMPLEMENTATION_PLAN_20260828.md`](docs/VALIDATED_REALTIME_PREVIEW_IMPLEMENTATION_PLAN_20260828.md)にあります
