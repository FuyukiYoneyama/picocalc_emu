...
- Validated Realtime Preview: **VRP-0（contract／fixture／WSLg capability／baseline）完了、実装本体は未着手**。権威あるfirmware PASS済みの同一BINと同一`picocalc-run`だけを対話表示する計画です。VRP-1〜VRP-4は`picotetris-opt1b`＋`picoedit-r1`で開始し、正式な1倍qualified昇格には別途NES-class target（VRP-NES-0）が必要です。現時点ではpreview機能や1倍動作をsupportedとみなしません。契約fixtureは[`docs/validated-realtime-preview/`](docs/validated-realtime-preview/)に、現行計画は[`docs/VALIDATED_REALTIME_PREVIEW_IMPLEMENTATION_PLAN_20260828.md`](docs/VALIDATED_REALTIME_PREVIEW_IMPLEMENTATION_PLAN_20260828.md)にあります
