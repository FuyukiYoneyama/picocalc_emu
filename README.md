...
- Validated Realtime Preview: **VRP-0／VRP-1完了（contract／fixture／WSLg capability／baseline／receipt生成・admission）**。`picocalc.py preview`は権威あるfirmware PASS済みの同一BIN・同一runnerを再検証し、GUIを起動せずadmitted descriptorを出力します。preview IPC本体、GUI、audio、1倍qualified昇格は未着手です。VRP-2〜VRP-4は`picotetris-opt1b`＋`picoedit-r1`で進め、正式な1倍qualified昇格には別途NES-class target（VRP-NES-0）が必要です。現時点ではpreview機能や1倍動作をsupportedとみなしません。契約fixtureとVRP-1記録は[`docs/validated-realtime-preview/`](docs/validated-realtime-preview/)に、現行計画は[`docs/VALIDATED_REALTIME_PREVIEW_IMPLEMENTATION_PLAN_20260828.md`](docs/VALIDATED_REALTIME_PREVIEW_IMPLEMENTATION_PLAN_20260828.md)にあります
