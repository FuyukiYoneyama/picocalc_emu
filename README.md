...
- Validated Realtime Preview: **VRP-0／VRP-1完了、VRP-2 backend API進行中**。`picocalc.py preview`は権威あるfirmware PASS済みの同一BIN・同一runnerを再検証し、admitted descriptorを出力します。backendの`--preview-api`は固定PCRP IPC、UART0 TX/RX、RGB565 frame、reset/quit、pacer status、fail-closed入力を実装済みで、batch／machine API／preview APIのsessionはbackendの`src/session.rs`へ共有分離済みです。preview statusとmachine APIの`preview`観測domainにはUART／framebuffer／unsupported-MMIO／audio-sinkのversioned projection/digestがあり、board-backed synthetic UART fixtureについてbatch／machine／previewを同一cycleで比較するreport-compatible observation digest smoke gate（初期RGB565 LCD frameを含む）をローカル確認済みです。ただしregistered target admissionへ接続した完全な同一virtual cycle gate、GUI、PicoCalc本体スキン、bounded audio、1倍qualified昇格は未実装です。VRP-2の完全gateを完了するまでpreview機能をsupportedとみなしません。VRP-2〜VRP-4は`picotetris-opt1b`＋`picoedit-r1`で進め、正式な1倍qualified昇格には別途NES-class target（VRP-NES-0）が必要です。契約fixtureとVRP-1記録は[`docs/validated-realtime-preview/`](docs/validated-realtime-preview/)に、現行計画は[`docs/VALIDATED_REALTIME_PREVIEW_IMPLEMENTATION_PLAN_20260828.md`](docs/VALIDATED_REALTIME_PREVIEW_IMPLEMENTATION_PLAN_20260828.md)にあります
