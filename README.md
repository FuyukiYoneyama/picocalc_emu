...
- Validated Realtime Preview: **提案レビューと実装計画を完了、実装は未着手**。権威あるfirmware PASS済みの同一BINと同一`picocalc-run`だけを対話表示する計画です。VRP-0〜VRP-4は`picotetris-opt1b`＋`picoedit-r1`で開始し、正式な1倍qualified昇格には別途NES-class target（VRP-NES-0）が必要です。現時点ではpreview機能や1倍動作をsupportedとみなしません。現行計画は[`docs/VALIDATED_REALTIME_PREVIEW_IMPLEMENTATION_PLAN_20260828.md`](docs/VALIDATED_REALTIME_PREVIEW_IMPLEMENTATION_PLAN_20260828.md)です
